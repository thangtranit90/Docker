...
```
docker build -t labs/base ./labs/images/base
```
...
📋 List all the images you have - then filter them for images starting with 'labs'.
...
# and filter for the labs images:
...
```
docker image ls --filter reference=ubuntu --filter reference=labs/base
```
...
📋 Build an image called `labs/curl` from the `labs/images/curl` Dockerfile.

<details>
  <summary>Not sure how?</summary>

```
docker build -t labs/curl ./labs/images/curl
```
...
docker run labs/curl docker.labs.co

# to use curl you need to specify the curl command:
docker run labs/curl curl --head docker.labs.co
```
...
```
docker build -t labs/curl:v2 -f labs/images/curl/Dockerfile.v2 labs/images/curl
```
...
```
docker run labs/curl:v2 --head docker.labs.co
```

> The `--head` argument and URL in the container command gets passed to the entrypoint

📋 List all the `labs/curl` images to compare sizes.
...
```
docker build -t labs/web ./labs/images/web
```
